...
{		
    ...
    private int currentIpd;
    public int CurrentIpd
    {
        get { return currentIpd; }
        set { if (value >= 0) currentIpd = value; }
    }
	...
	new void Start()
	{
	    ...
	    currentIpd = (int)(ipd * 1000f);
        Debug.Log("CurrentIPD: " + currentIpd);
	}
    ...
    {
        if (Input.GetButtonDown("DepthIncrease"))
            Debug.Log("Hello");

        const int depthStep = 1;

        if (Input.GetButton("DepthIncrease"))
        {
            Debug.Log("Increasing Depth");
            CurrentIpd += depthStep;
            ipd = CurrentIpd * 0.001f;
        }

        else if (Input.GetButton("DepthDecrease"))
        {
            Debug.Log("Decreasing Depth");
            CurrentIpd -= depthStep;
            ipd = CurrentIpd * 0.001f;
        }
    }
